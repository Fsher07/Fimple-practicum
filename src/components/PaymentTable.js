...
function PaymentTable(props, ref) {
  ...
  const compoundInterestCalculation = () => {
    for (let i = 1; i <= installments; i++) {
      interest =
        balance * (1 + interest_rate / 100) ** (interval / 30) - balance;
  ...
  return tableSwitcher ? (
    <div className='payment-table'>
      <button className='closeBtn' onClick={() => setTableSwitcher(false)}>
        X
      </button>
      <table>
        <thead>
        ...
        <tbody>
          {trigger === "compound"
            ? compoundInterestCalculation().map((row) => row)
            : simpleInterest().map((row) => row)}
        </tbody>
      </table>
    </div>
  ) : (
